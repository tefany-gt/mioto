...
const WorkshopList: React.FC<WorkshopListProps> = ({ onRequireAuth, viewMode = 'horizontal', onViewAll, onNavigate, filter, onOpenDetails }) => {
  ...
  const containerClass = viewMode === 'horizontal' 
    ? "flex overflow-x-auto gap-4 pb-4 no-scrollbar -mx-4 px-4 snap-x min-h-[200px] md:grid md:grid-cols-2 lg:grid-cols-3 md:mx-0 md:px-0 md:overflow-visible"
              ...
              >
                <button 
                    ...
                    className={`absolute top-3 right-3 z-10 p-1.5 rounded-full shadow-sm transition-colors ${
                        isFavorite ? 'bg-red-50 text-red-500' : 'bg-white/90 text-gray-400 hover:text-red-500 hover:bg-white'
                    }`}
                >
